...
ranges <- data.frame (
  x = c(1,3,5,10,10,10),
  y = c(1,3,5,10,15,20))
MSFT_test <- getSymbols("MSFT", from = start_date, to = end_date, auto.assign = FALSE)

getting_data <- function(start_date,end_date){
  
  ff_data <- read.table("ff_data.csv",header=TRUE, sep=",")
  tickers <- c("MSFT", "GOOGL", "VZ", "BIDU","VT")
...
fitting_train_factors_VT <- function(train_table, test_table){
  train_regression <- lm(VT - RF ~ Mkt.RF + SMB + HML + RMW + CMA, train_table)
  table_name <- deparse(substitute(train_table))
  ...
  b <- sum(test_table$VT)
  c <- b - sum_a 
  print(paste("total predicted return of VT:", sum_a))
  print(paste("total actual return of VT:", b))
  print(paste("Actual - predicted return of VT:", c))
  print(" ")
  return(c)
} 

VT_data <-function(train_table1,train_table2,train_table3,train_table4,train_table5,train_table6,test_table1,test_table2,test_table3,test_table4,test_table5,test_table6){
  fitting_train_factors_VT(train_table1 ,test_table1)
  fitting_train_factors_VT(train_table2, test_table2)
  fitting_train_factors_VT(train_table3, test_table3)
  fitting_train_factors_VT(train_table4, test_table4)
  fitting_train_factors_VT(train_table5, test_table5)
  fitting_train_factors_VT(train_table6, test_table6)
}
VT_div <- VT_data(train_table1,train_table2,train_table3,train_table4,train_table5,train_table6,test_table1,test_table2,test_table3,test_table4,test_table5,test_table6)
